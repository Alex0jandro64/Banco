...
public class EmailServicioImpl implements IEmailServicio {
	...
	public void enviarEmailRecuperacion(String emailDestino, String nombreUsuario, String token) {
		   ...
		   try {
	            ...
	            helper.setFrom("alejandroalcerreca3@gmail.com"); //AQUI VA EL EMAIL DEL .PROPERTIES
	            ...
	            String cuerpoMensaje = String.format(
	    				...
	    				+ " font-family: Sans-serif'> <h1 style='color:#192255'>Restablecer contraseña<b style='color:#ff9900'> Banco Final</b></h1>"
    ...
    public void enviarEmailConfirmacion(String emailDestino, String nombreUsuario, String token) {
        try {
            ...
            String cuerpoMensaje = String.format(
            		"﻿<!DOCTYPE html> <html lang='es'> <body> <div style='width: 600px; padding: 20px; border: 2px solid #ff9900; border-radius: 12px;"
    	    				+ " font-family: Sans-serif'> <h1 style='color:#192255'>Crear Cuenta<b style='color:#ff9900'> Banco Final</b></h1>"
    	    				+ " <p style='margin-bottom:25px'>Estimado/a&nbsp;<b>%s</b>:</p> <p style='margin-bottom:25px'>"
    	    				+ "Recibiste este correo porque esta intentando crear una cuenta. Haz clic en el botón que aparece a continuación para terminar de crear su cuenta.</p>"
    	    				+ " <a style='padding: 10px 15px; border-radius: 20px; background-color: #285845; color: white; text-decoration: none' href='%s' target='_blank'>Crear Cuenta</a>"
    	    				+ " <p style='margin-top:25px'>Si no solicitaste crear la cuenta, puedes ignorar este correo de forma segura.</p>"
    	    				+ " <p>Gracias por utilizar nuestros servicios.</p> </div> </body> </html>",
